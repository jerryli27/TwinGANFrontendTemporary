...
from interface import visualization_utils_color as vis_util
import util_io
import util_misc
...
def crop_by_category(image,
  ...
  while num_boxes < min(max_boxes_to_draw, boxes.shape[0]) and i < min(max_boxes_to_draw, boxes.shape[0]):
    if (scores is None or scores[i] > min_score_thresh) and classes[i] == category_to_crop:
      ...
      ymin_absolute, xmin_absolute, ymax_absolute, xmax_absolute = int(ymin * img_height), int(xmin * img_width), int(ymax * img_height), int(xmax * img_width)
      expanded_xywh = util_misc.unevenly_expand_xywh(
        xmin_absolute, ymin_absolute, xmax_absolute-xmin_absolute, ymax_absolute-ymin_absolute, img_width, img_height,
...
def main(_):
  fd = FaceDetector()
  fd.crop_face_and_save('../data/face_detection/sample_img.jpg', '../data/face_detection/sample_img_face_%d.jpg')
